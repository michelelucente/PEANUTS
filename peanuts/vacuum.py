...
def vacuum_evolved_state(nustate, pmns, DeltamSq21, DeltamSq3l, E, L, antinu=False):
    ...
    evol = np.dot(
        np.dot(
            np.dot(r23, delta),
            np.dot(
                Upert(DeltamSq21, DeltamSq3l, pmns.theta12, pmns.theta13, pmns.U, E, 0.0, l, 0.0, 0.0, 0.0, antinu),
...
def Pvacuum(nustate, pmns, DeltamSq21, DeltamSq3l, E, L, antinu=False, massbasis=True):
    ...
    evol = np.dot(
        np.dot(
            np.dot(r23, delta),
            np.dot(
                Upert(DeltamSq21, DeltamSq3l, pmns.theta12, pmns.theta13, pmns.U, E, l, 0.0, 0.0, 0.0, 0.0, antinu),
                delta.conjugate().transpose(),
            ),
        ),
        r23.transpose(),
