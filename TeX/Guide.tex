...
If the detector is placed underground, we can define a ``detector shell'' at the detector radial distance. In our example SNO was placed 2 km underground, so we can define $r_\text{SNO} = 1 - 2 \text{ km} /r_\text{E}$, where $r_\text{E}$ is the Earth radius, cf. Figure~\ref{fig:scheme_detector_shell}.
...
\begin{equation}
	x_i = \sqrt{r_i^2 - \sin \eta^2}, \hspace{1cm} \text{ for $i$ such that} \hspace{1cm} r_i > \sin \eta.
\end{equation}
