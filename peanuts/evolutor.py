...
def FullEvolutor(density, DeltamSq21, DeltamSq3l, pmns, E, eta, depth, antinu):
    ...
    if 0 <= eta < pi/2:
        ...
        evolutor =  np.dot(np.dot(np.dot(r23, delta), np.dot(evolutor_half_detector, evolutor_half_full.transpose())), np.dot(delta.conjugate(), r23.transpose()))
        return evolutor
    ...
    # If pi/2 <= eta <= pi we approximate the density to the constant value taken at r = 1 - h/2
        ...
        evolutor = np.dot(np.dot(np.dot(r23, delta), np.dot(Upert(DeltamSq21, DeltamSq3l, pmns, E, Deltax, 0, n_1, 0, 0, antinu), delta.conjugate())), r23.transpose())
        return evolutor
